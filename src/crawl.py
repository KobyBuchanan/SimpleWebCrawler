...
def get_urls_from_html(html:str, base_url:str):
    ...
    links = []
    for link in a_tags:
        absolute_url = urljoin(base_url,link.attrs.get('href'))
...
def get_images_from_html(html:str, base_url:str):
    ...
    links = []
    for link in a_tags:
        absolute_url = urljoin(base_url,link.attrs.get('src'))
        links.append(absolute_url)
    return links

def extract_page_data(html:str, base_url:str):
    url = base_url
    header = get_h1_from_html(html)
    first_paragraph = get_first_paragraph_from_html(html)
    outgoing_links = get_urls_from_html(html,base_url)
    image_links = get_images_from_html(html,base_url)
    data = {'url':url,
            'h1':header,
    ...
    async def get_html(self, url):
        try:
            async with self.session.get(
                url, headers={"User-Agent": "BootCrawler/1.0"}) as response:
